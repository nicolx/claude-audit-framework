...
git -C .claude/framework checkout v1.2.0
...
```bash
cd .claude/framework && git fetch --tags && git checkout v1.2.0 && cd ../..
bash .claude/framework/scripts/init-project.sh     # required — refreshes the command copies
git add .claude/ && git commit -m "chore: update claude-audit-framework to v1.2.0"
```
