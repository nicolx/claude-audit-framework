...
git -C .claude/framework checkout v1.7.1
...
# 2. Move to it
cd .claude/framework && git fetch --tags && git checkout v1.7.1 && cd ../..
...
git add .claude/ CLAUDE.md && git commit -m "chore: update claude-audit-framework to v1.7.1"
```
...
The conformance rules ship *inside* each version, so checking out `v1.7.1` gets you `v1.7.1`'s
